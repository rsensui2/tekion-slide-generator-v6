---
name: design-setup
description: "TEKION Slide Generator v6 のブランドデザインを設定するオンボーディングスキル。初めて使う会社が自社のデザインフォーマット（ロゴ・ロゴ位置・配色・フォント・トーン）を登録できる。素材ファースト: ロゴ画像、スライドマスターの PPTX、ブランドガイドラインのスクショ、既存デッキなどを受け取って解釈し、プリセット・ロゴ・設定ファイルを適切な場所に配置する。既存プリセットの改善（ロゴ位置変更・色修正など）と、スライドに繰り返し登場させるキャラクター・写真のアセットライブラリ登録にも対応。「デザインを設定したい」「ブランドをカスタマイズしたい」「自社カラーに設定して」「ロゴを設定して/位置を変えて」「この会社用のフォーマットを作って」「キャラクターを登録したい」で発動。"
---
...
4. **アセット登録** — キャラクター・人物写真・製品写真など、スライドに繰り返し登場させたい
   画像の登録（「キャラクターを登録したい」「りょーこみたいなのを自社でもやりたい」等）
   → 下の「アセットライブラリへの登録」へ

## アセットライブラリへの登録

ロゴ以外の「よく使う参照画像」の常設置き場。v6 の Phase 6 が自動で読んで、該当スライドの
参照画像に割り当てる。
...
手順:
1. 画像をもらい `${ASSETS_DIR}/` に分かりやすいファイル名でコピー（キャラクターは
   バストアップ + 全身/情景の2枚あると使い分けられる、と案内する）
2. 「これは何か」「どういうスライドで登場させたいか」を1問で聞く
3. `${ASSETS_DIR}/assets.md` に追記する。フォーマット（1アセット = 1見出し）:

```markdown
## <名前>（<呼び名・別名>）

- ファイル: `<file1>` — <どの用途向けか>
- これは何: <キャラクター/人物/製品の説明>
- 使いどころ: ユーザーの指示に「<呼び名>」とあれば必ず参照。<登場させたいスライドの種類>
- 注意: 参照した場合は「参照画像のキャラクターの顔立ち・髪型・雰囲気を忠実に保つこと」を
  プロンプトに明記する。
```

完了時に「次回から『◯◯を出して』と言うだけでスライドに登場します」と伝える。
